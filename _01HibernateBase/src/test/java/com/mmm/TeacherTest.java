...
public class TeacherTest {
    ...
    public void addTeacher(){
        ...
        //teacher.setId(2);//id,如果数据库id是自增的 可以省略，不是自增要手动填写
        teacher.setName("小菠萝");
        //06.持久化操作
        System.out.println("***************");
        session.save(teacher);//indentity产生sql语句
        System.out.println("***************");
        //factory.close();
    }
     ...
     *         如果有，直接返回，不去访问数据库，
     *         如果没有，而且我们配置了2级缓存，name会去2级缓存中查询，
     *         如果2级缓存没有数据，就去访问数据库。
     *      03.如果数据库中有对应的数，直接返回
     *          数据库中没有对应的数据，则返回null
     *
     ...
     *      数据库红没有对应的数据，则抛出ObjectNotFoundException 异常
     *
     */
    @Test
